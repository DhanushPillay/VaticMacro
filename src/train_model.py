...
def compute_metrics(y_true, y_pred) -> dict:
    """FAANG: single source for R2/MAE/RMSE used by train + benchmarks."""
...
def naive_baseline_mae(y_true) -> float:
...
def train(df):
    ...
    X = training_df.drop(
        ["Date", "CPI", "target_future_inflation"], axis=1, errors="ignore"
    )
    ...
    ridge_grid = GridSearchCV(
        ridge_pipe, ridge_param_grid, cv=tscv, scoring="r2", n_jobs=-1
    )
    ridge_grid.fit(X, y)
    ...
    print(
        f"Best Ridge R2: {ridge_r2:.4f} (alpha={ridge_grid.best_params_['model__alpha']})"
    )

    # 1b. Ridge with mutual_info scoring
    ridge_mi_grid = GridSearchCV(
        ridge_mi_pipe, ridge_mi_param_grid, cv=tscv, scoring="r2", n_jobs=-1
    )
    ...
    print(
        f"Best Ridge(MI) R2: {ridge_mi_r2:.4f} (alpha={ridge_mi_grid.best_params_['model__alpha']}, k={ridge_mi_grid.best_params_['select__k']})"
    )
    ...
    print(
        f"Best ElasticNet R2: {en_r2:.4f} (alpha={en_grid.best_params_['model__alpha']}, l1_ratio={en_grid.best_params_['model__l1_ratio']})"
    )
    ...
    poly_grid = GridSearchCV(
        poly_pipe, poly_param_grid, cv=tscv, scoring="r2", n_jobs=-1
    )
    poly_grid.fit(X, y)
    ...
    print(
        f"Best Ridge(Poly) R2: {poly_r2:.4f} (alpha={poly_grid.best_params_['model__alpha']}, k={poly_grid.best_params_['select__k']})"
    )
    ...
    try:
        ...
        lgbm_grid = GridSearchCV(
            lgbm_pipe, lgbm_param_grid, cv=tscv, scoring="r2", n_jobs=-1
        )
        lgbm_grid.fit(X, y)
    ...
    metrics_output = {
        ...
        "metrics": [
            ...
            {"name": "ElasticNet", "r2_mean": en_r2, "mae": en_mae, "rmse": en_rmse},
            {
                "name": "Ridge(Poly)",
                "r2_mean": poly_r2,
                "mae": poly_mae,
            ...
            {"name": "Random Forest", "r2_mean": rf_r2, "mae": rf_mae, "rmse": rf_rmse},
            {"name": "XGBoost", "r2_mean": xgb_r2, "mae": xgb_mae, "rmse": xgb_rmse},
            {
                "name": "LightGBM",
                "r2_mean": lgbm_r2,
                "mae": lgbm_mae,
