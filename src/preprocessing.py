...
def load_and_clean_data(path, start_date="2000-01-01", end_date="2030-12-31"):
    ...
    if "observation_date" in df.columns:
        ...
        date_candidates = [
            col for col in df.columns if "date" in col.lower() or "label" in col.lower()
        ]
        if date_candidates:
            date_col = date_candidates[0]
        else:
            raise ValueError(
                f"No date column found in {filename}. Columns: {list(df.columns)}"
            )
...
def _handle_pivot_format(df, filename, start_date="2000-01-01", end_date="2030-12-31"):
    ...
    melted["observation_date"] = pd.to_datetime(
        melted["Year/Month"] + " " + melted["Year"], errors="coerce"
    )
    ...
    melted = melted[
        (melted["observation_date"] >= start) & (melted["observation_date"] <= end)
    ]
...
def _handle_year_columns_format(
    df, filename, start_date="2000-01-01", end_date="2030-12-31"
):
    """
    ...
    # Melt the dataframe
    melted = df.melt(
        id_vars=[id_col], value_vars=year_cols, var_name="Year", value_name="value"
    )
